##---------------------------------------------------------------------
## Program: Accessibility_Boundary.py
## Version: 1.0
##
## Author: Nick Joslyn
...
##  Solves for state boundary metric.
##---------------------------------------------------------------------
...
supplyFileExcel = '../../Facilities/ClinicInfo.csv'
...
DemandInformation = np.genfromtxt(demandFileExcel, delimiter=',', dtype=str)
SupplyInfo = np.genfromtxt(supplyFileExcel, delimiter=',', dtype=str)
...
for i in range(numberOfDemandNodes):
    withinState, = np.where(SupplyInfo[:,2] == DemandInformation[i,1])
    if (len(withinState) > 0):
        stateDistances = []
        for j in withinState:
            stateDistances.append(solutionMatrix[i,j])
        closestSupplyLocation = np.amin(stateDistances)
...
for national in demandDistances:
    ...
    if (national < 0):
        nationalIsolated[0] = nationalIsolated[0] + 1
        nationalIsolated[1] = nationalIsolated[1] + DemandNumbers[counter,1]
    counter = counter + 1
...
northeast4 = [0,0]
northeast5 = [0,0]
northeastIsolated = [0,0]

for index in NEindexes:
    if (0 <= demandDistances[index] <= 0.5):
    ...
    if (demandDistances[index] < 0):
        northeastIsolated[0] = northeastIsolated[0] + 1
        northeastIsolated[1] = northeastIsolated[1] + DemandNumbers[index, 1]
...
for index in MWindexes:
    ...
    if (demandDistances[index] < 0):
        midwestIsolated[0] = midwestIsolated[0] + 1
        midwestIsolated[1] = midwestIsolated[1] + DemandNumbers[index, 1]
...
for index in Sindexes:
    ...
    if (demandDistances[index] < 0):
        southIsolated[0] = southIsolated[0] + 1
        southIsolated[1] = southIsolated[1] + DemandNumbers[index, 1]
...
for index in Windexes:
    ...
    if (demandDistances[index] < 0):
        westIsolated[0] = westIsolated[0] + 1
        westIsolated[1] = westIsolated[1] + DemandNumbers[index, 1]
